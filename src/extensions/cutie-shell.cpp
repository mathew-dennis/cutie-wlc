...
// ---  New: GPU Texture Access ---
QOpenGLTexture* CutieShell::textureForToplevel(ForeignToplevelHandleV1* toplevel)
{
    if (!toplevel)
        return nullptr;

    CwlView* view = toplevel->view();
    if (!view)
        return nullptr;

    return view->getTexture(); // Assumes getTexture() is already defined
}
